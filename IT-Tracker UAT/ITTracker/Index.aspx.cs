...
namespace ITTacker
{
    public partial class Index : System.Web.UI.Page
    {
        ...
        [WebMethod(EnableSession = true)] 
        public static string GetBugDetails(string pageVal, string pageval1)
        {
            DataSet ds;
            string str = "";
            int i = 0;
            ITTracker.ITService.ITServiceClient obj1 = new ITTracker.ITService.ITServiceClient();
            ds = obj1.TrackerSelect("REPORTVIEW", pageVal, pageval1, "", "");
            try
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
                    for (i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                       str = str + ds.Tables[0].Rows[i][0].ToString() + "~" + ds.Tables[0].Rows[i][1].ToString() + "~" + ds.Tables[0].Rows[i][2].ToString() + "~" + ds.Tables[0].Rows[i][3].ToString() + "~" + ds.Tables[0].Rows[i][4].ToString() + "~" + ds.Tables[0].Rows[i][5].ToString() + "µ";
