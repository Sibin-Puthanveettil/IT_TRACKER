...
function CRFModalLoad() {
    var usr = $("[id*=hdUserId]").val();
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFHODApprove.aspx/getFillData",
        data: "{pageVal:'ChangeUserCrfList', pageval1 :'" + usr + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
...
function UserListLoad() {
    ...
    $('#txtCurrUser').val("");
    var usr = $("[id*=hdUserId]").val(); 
    var draftNo = $("#ddlCrfList").val();  
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFHODApprove.aspx/getFillData",
        data: "{pageVal:'GetEmpAgainstDept', pageval1 :'" + draftNo + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            $('#ddlToUser ').append($("<option></option>").val('-1').html('Choose User'));
            $.each(Result, function (key, value) {
...
function ChangeUserConfirm() {
    ...
    if ($("#txtModRemarks").val()=="") {
        swal.fire("Please Enter Remarks");
        return false;
    }
    if ($("#ddlToUser").val()==-1) {
        swal.fire("Please Select New User");
        return false;
    }
    Data = $("#txtModRemarks").val() + 'µ' + crfid + 'µ' + $("[id*=hdUserId]").val() + 'µ' + $("#ddlToUser").val() + 'µ' + $("#ddlToUser option:selected").text();

    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFHODApprove.aspx/ConfirmChangeUser",
        data: "{pageVal:'ChangeUserConfirm', pageval1 :'" + Data + "',pageval2:'" + $('#txtUserId').val()+"',pageval3:''}",
        dataType: "json",
        async: false,
        success: function (Result) {
            $('.block-ui').addClass('clear');
