...
function getalreadyAssignedTester() {
    var crf = $("#dd_subcrf2").val();
    $("#dd_tester").empty();
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "team_module_dev.aspx/getData",
        data: "{pageVal:'TEAMMODULE',pageval1 :'TESTERLIST',pageval2:'" + crf + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            $('#dd_tester').append($("<option selected></option>").val("-1").html("Choose already assigned test engineer.!"));
            $.each(Result, function (key, value) {
                $('#dd_tester').append($("<option></option>").val(value.id).html(value.name));
            });
        }
    });
...
function subcrfonchange1() {
    $("#dd_subcrf2").empty();
    var crf = $("#txtmodule1").val();
    ...
    setTimeout(function () {
       ...
       // var crf = $("#txtmodule1").val();
        ...
        $.ajax({
            ...
            success: function (Result) {
                Result = Result.d;
                $('#dd_subcrf2').append($("<option selected></option>").val("-1").html("Select Sub CRF"));
                $.each(Result, function (key, value) {
                    $('#dd_subcrf2').append($("<option></option>").val(value.id).html(value.name));
                });
            }
        });
...
function gettestingTA(crf) {
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "team_module_dev.aspx/getTableData",
        data: "{pageVal:'TestingTADtls2',pageval1 :'" + crf + "',pageval2:''}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
           // alert(Result);
            fillTestingTable(Result);
        }
    });
}
...
function fillTestingTable(data) {
  
    data = data.replace('¶µΘ', '¶');
    data = data.replace('µΘ', '¶');
    ...
    if ($("#tabChange2 tr").length == 0) {
        $("#tabChange2").empty();
        $('#tabChange2').append('<tr style="color:black; background-color:honeydew"><th class="text-center">REQ ID</th><th class="text-center">TEST LEAD</th><th class="text-center">TEST ENGINEER</th><th class="text-center">PROJECT TYPE</th><th class="text-center">PHASE</th><th class="text-center">DESCRIPTION</th><th class="text-center">TEST STARTDATE</th><th class="text-center">TEST ENDDATE</th><th class="text-center">WORK HOURS</th></tr>');

    }

    for (i = 0; i < valData.length-1; i++) {
        valData1 = valData[i].split('µ');
        if (valData1.length == 10) {
            reqid = valData1[0];

            $('#tabChange2').append('<tbody><tr class="text-center" style="background-color:#fff" >' +
                '<td>' + valData1[0] + '</td>' +
                '<td>' + valData1[1] + '</td>' +
                '<td>' + valData1[2] + '</td>' +
                '<td>' + valData1[5] + '</td>' +
...
function fillTLTable(data) {
    ...
    if ($("#tabChange1 tr").length == 0) {
        $("#tabChange1").empty();
        $('#tabChange1').append('<tr style="color:black; background-color:honeydew""><th class="text-center">RequestId</th><th class="text-center">TechLead</th><th class="text-center">TechLeadName</th></tr>');
    }
    ...
    for (i = 0; i < valData.length - 1; i++) {
        ...
        $('#tabChange').append('<tbody><tr class="text-center" style="background-color:white" >' +
    ...
    for (i = 0; i < valData.length - 1; i++) {
        ...
        $('#tabChange1').append('<tbody><tr class="text-center" style="background-color:white" >' +
...
function ConfirmTesterChange() {

       var usr = $("[id*=hdUserId]").val();
       var remark = $("#txtRemarks3").val();
       var crf = $("#txtmodule1").val();
       
    var subcrf = $("#dd_subcrf2").val();
    var stdt = $("#lblstaDt1").val();
    var enddt = $("#lblTarDt1").val();
    var alredytester = $("#dd_tester").val();
    var changeto = $("#txtdev1").val();
    ...
    if (alredytester == "") {
        Swal.fire("Please Choose already assigned Tester.");
        return false;
    }
    if (changeto == "") {
        Swal.fire("Please Choose new Test Engineer");
        return false;
    }
    
    input = crf + "^" + subcrf + "^" + usr + "^" + remark + "^" + stdt + "^" + enddt + "^" + alredytester + "^" + changeto;
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            url: "team_module_dev.aspx/Confirm",
            data: "{pageVal:'TEAMMODULE',pageval1 :'CONFIRMTESTERS',pageval2:'" + input + "'}",
            dataType: "json",
            success: function (Result) {
                Result = Result.d;
                Swal.fire(Result);
...
// for teters..................................................................................................................................
function fillCRFDetailsTesters() {
    $('#txtmodule1').empty();
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "team_module_dev.aspx/getData",
        data: "{pageVal:'TEAMMODULE',pageval1 :'GETCRFFORTESTERS',pageval2:''}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            $('#txtmodule1').append($("<option selected></option>").val("-1").html("Choose CRF.."));
            $.each(Result, function (key, value) {
                $('#txtmodule1').append($("<option></option>").val(value.id).html(value.name));
            });
        }
    });
} 
...
function fillTesters() {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        url: "team_module_dev.aspx/getData",
        data: "{pageVal:'TEAMMODULE',pageval1 :'GETTESTER',pageval2:''}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            $('#txtdev1').append($("<option selected></option>").val("-1").html("Choose Test Engineer "));
            $.each(Result, function (key, value) {
                $('#txtdev1').append($("<option></option>").val(value.id).html(value.name));
            });
        }
    });
}
