...
function viewBugReport() {
    ...
    var wh = txt_TASrtDate + 'æ' + txt_TarDate + 'æ' + v_tester + 'æ';
    //alert(wh);
    encryptUrl(wh);
...
function GetTester() {
    ...
    $.ajax({
        ...
        success: function (Result) {
            //alert(Result);
            Result = Result.d;
            $('#ddlTester').append($("<option ></option>").val("-1").html("Select Tester"));
...
function encryptUrl(dtdata) {
    $.ajax({
        ...
        success: function (Result) {
            ...
            window.open("QA_Team_Availability.aspx?mnuId=" + Result + "");
