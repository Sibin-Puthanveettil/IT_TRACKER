...
function pendingWork() {
    ...
    $.ajax({
        ...
        url: "dailyupdate_dba.aspx/selectdata",
...
function AssignedProjectLoad() {
   // alert('haii');
    ...
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "dailyupdate_dba.aspx/getFillData",
        data: "{pageVal:'Getsr', pageval1 :'" + usr + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
...
function WorkDescLoad() {
    ...
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "dailyupdate_dba.aspx/getDescOfWork",
        data: "{pageVal:'DBAstatus', pageval1 :'" + usr + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
...
function display(data) {
    ...
    if ($("#tableData tr").length == 0) {
        $("#tableData").empty();
        $("#tableData").append('<tr style="background-color:#fdd935;color:#ed2023"><th class="text-center">SLNo.</th><th class="text-center">Assigned Work</th><th class="text-center">Work Status </th><th class="text-center">Detail Description</th><th class="text-center">Time</th><th class="text-center">Remarks</th><th class="text-center">Date</th><th class="text-center">Percentage</th><th class="text-center">Delete</th><tr>');
    }
    ...
    for (i = 0; i < valData.length - 1; i++) {
        ...
        $("#tableData").append('<tr id="row_' + (n + 1) + '">' +
            ...
            '<td class="text-center" style="display:none">db-' + valData1[1]+'</td>' +
            '</tr >');
...
function ConfirmNote() {
    ...
    if ($("#tableData tr").length == 0) {
        ...
        for (var i = 1; i < rowLength - 1; i++) {
            ...
            ItemDtls = ItemDtls + workId + '^' + DescId + '^' + table.rows[i + 1].cells[3].innerHTML + '^' + table.rows[i + 1].cells[4].innerHTML + '^' + table.rows[i + 1].cells[5].innerHTML + '^' + usr + '^' + table.rows[i + 1].cells[6].innerHTML + '^' + table.rows[i + 1].cells[7].innerHTML + '^' + table.rows[i + 1].cells[11].innerHTML+ '^-^-^$';
        ...
        $.ajax({
            ...
            url: "dailyupdate_dba.aspx/InsertData",
            ...
            success: function (result) {
                ...
                result = result.d;
                alert(result);
                window.open('dailyupdate_dba.aspx?crfid=1', '_self');
            },
