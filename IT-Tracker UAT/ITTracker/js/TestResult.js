$(window).on('load', function () {
    ...
    $('#applicableOption').prop('checked', false);
    $("#DivSuggestion").hide();
    CRFLoad();
    GetSeverity();
    GetEnvironment();
...
function shdetails(tn, sts) {
    ...
    if ($("[id*=hdEmpCheck]").val() == 0)//dev
    {
        if (status_id == 2 || status_id == 1 || status_id == 5) {

            $("#btnSubmitD").prop("disabled", false);
        }
        else {
            $("#btnSubmitD").prop("disabled", true);
        }
    }
    else {
        if (status_id == 6 || status_id == 3) {
            $("#btnSubmitD").prop("disabled", false);
        }
        else {
            $("#btnSubmitD").prop("disabled", true);
...
function gr() {
    ...
    $('#tblticket').find('tr').click(function () {
        ...
        if ($("[id*=hdEmpCheck]").val() == 0)//dev
        {
            var sts = $(this).find('td:eq(9)').text();
        }
        else {
            var sts = $(this).find('td:eq(10)').text();
        }
        shdetails(row, sts);
        //WorkLogView(row);
        //UserApprvTview(row);
...
function TestRequestFormLoaded(request_id) {


    $("#tblTestrqstfrm").empty();
    var filenm = "Testrqstfrm_" + $("[id*=hdUserId]").val() + "_" + request_id.replace(/[^a-zA-Z0-9]/g, '') + "_";
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFTestResult.aspx/getFileData",
        async: false,
        data: "{pageVal:'GetTestRequestForm', pageval1 :'" + request_id + "', pageval2 :'" + filenm + "'}",
        dataType: "json",
        success: function (Result) {
            ...
            if ($("#tblTestrqstfrm tr").length == 0) {
                $('#tblTestrqstfrm').append('<thead class="bg-success text-white"><tr><th scope="col">File No</th><th scope="col">File Name</th></tr></thead>');
            }
    ...
    $.ajax({
        ...
        success: function (Result) {
            if (Result.d.length > 0) {
                $('#tblticket').empty();
                $('#tblticket').append('<thead < tr style="color:black; background-color:honeydew" class="text-left" ><td scope="col">NO</td><td scope="col">Bug ID</td><td scope="col">Subject</td><td scope="col">Request No</td> <td scope="col">Bug Status</td> <td scope="col">Bug Date</td><td scope="col">Bug Reported</td><td scope="col">Closed Date</td><td scope="col">Test Stage</td><td scope="col">Developer Comment</td> </tr></thead><tbody>');
                for (var i = 0; i < Result.d.length; i++) {
                    ...
                    $('#tblticket').append('<tr><td>' + k + '</td>' +
                        ...
                        '<td>' + Result.d[i].BUGDate + '</td>' +
                        '<td>' + Result.d[i].BUGReported + '</td>' +
                        '<td>' + Result.d[i].BUGCloseDt + '</td>' +
                        '<td>' + Result.d[i].TestStage + '</td>' +
                        '<td>' + Result.d[i].DvlprCmnt + '</td>' +
                        '<td style="display:none">' + Result.d[i].sts_id + '</td>' +
                        '</tr>');
                }
                $('#tblticket').append(
                    '</tbody>');
...
function BugTview(req) {
    ...
    $.ajax({
        ...
        success: function (Result) {
            if (Result.d.length > 0) {
                $('#tblticket').empty();
                $('#tblticket').append('<thead < tr style="color:black; background-color:honeydew" class="text-left" ><td scope="col">NO</td><td scope="col">Bug ID</td><td scope="col">Subject</td><td scope="col">Request No</td> <td scope="col">Bug Status</td> <td scope="col">Bug Date</td><td scope="col">Bug Reported</td><td scope="col">Close Date</td><td scope="col">Developer Comment</td> </tr></thead><tbody>');
                for (var i = 0; i < Result.d.length; i++) {
                    k = i + 1;
                    $('#tblticket').append('<tr><td>' + k + '</td>' +
                        '<td>' + Result.d[i].BugID + '</td>' +
...
var st_values = "";
function GetTestStatus(sts) {
    var rq = $("[id*=hdRqstID]").val();
    $('#ddlTestStatus').empty();
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFTestResult.aspx/getFillData",
        data: "{pageVal:'GetTestStatus', pageval1 :'" + sts + "', pageval2 :'" + rq + "'}",
        async: false,
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            //alert(Result);
            $('#ddlTestStatus').append($("<option ></option>").val("-1").html("Choose Status"));
            $.each(Result, function (key, value) {
                st_values = st_values + '^' + value.id;
...
function GetBugStatusAgainstReq() {
    ...
    $.ajax({
        ...
        success: function (Result) {
            Result = Result.d;
            if (Result == '111') {
                alert("Initial test is not started..!");
            }
            else {
                BugDetails = Result;
                var Bugd = BugDetails.split('±');
                $("[id*=hdTestCheck]").val(Bugd[0]);
                $("[id*=hdBugStatus]").val(Bugd[1]);
            }
        }
    });
...
function tsterdelaychk(req, CRFID) // collecting delay date in db 
{
    //alert("delayreason");
    var usr = $("[id*=hdUserId]").val();
    ...
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFTestResult.aspx/tsterdlychk",
        data: "{p_flag:'tsterdelaychk', p_pageval :'" + req + "',p_parval1 :'" + CRFID + "'}",
        dataType: "json",
...
function delayreason() //collecting Delay reson in DB
{
    //alert("delayreason");
    var usr = $("[id*=hdUserId]").val();
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        url: "CRFTestResult.aspx/getFillData",
        data: "{pageVal:'delayreason', pageval1 :'" + usr + "',pageval2 :''}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            $.each(Result, function (key, value) {
                $('#testdelay').append($("<option></option>").val(value.id).html(value.name));
            });
        }
    });
}

function delaydata()//Inserting DelaY reason in DB
{
    //alert("delayreason");
    var usr = $("[id*=hdUserId]").val();
    var dlydata = $("#ddlCrf").val() + '^' + $("[id*=hdRqstID]").val() + '^' + $("[id*=hdUserId]").val() + '^' + $('#ddlTestStatus').val() + '^' + $('#testdelay').val() + '^' + $("#txtSuggestion1").val()

    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "CRFTestResult.aspx/insrtdlydata",
        data: "{p_flag:'delaydata', p_pageval :'" + dlydata + "',p_parval1 :''}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            ...
            if (Result == "111") {
                ...
                console.log("delaydata() : sumbitted")

            }
            else {

                console.log("delaydata() :" + Result)
...
function fillCRFData() {
    ...
    if ($("[id*=hdEmpCheck]").val() == 0) {
        ...
        if ($("[id*=hdTestCheck]").val() != 0) {
            ...
            GetTestStatus($("[id*=hdTestCheck]").val());
        } else {
...
function TestResultconfirm() {
    ...
    if ($('#ddlTestStatus').val() != 4 && $("[id*=hdEmpCheck]").val() == 1) {
        if (st_values.includes('4')) {
            alert("Please Start your Intial Testing !");
            return false;
        }
    }
    ...
    //else if (TestStatus == 2 || TestStatus == 8) {
    //    alert("If You have any bug  Please enter your bugs.!");
    ...
    if ($('#r_and_d').prop('checked')) {
        if ($("#txtSuggestion").val() == "") {
            Swal.fire("Please enter the suggestion.!");
            return false;
        }
        else sugges = $("#txtSuggestion").val();
    }
    else sugges = "Nil";

    //if ($("#txtSuggestion1").val() == "") {
    //    Swal.fire("Please enter the Delay suggestion.!");
    //    return false;
    //}
    debugger;
    if (DelayStatus == 1) {
        ...
        if ($("#testdelay").val() == "-1") {
            ...
            Swal.fire("Please Update the Delay Reason!");
            return false;
        }
    ...
    if (DelayStatus == 1) {
        ...
        if ($("#txtSuggestion1").val().trim().length < 10) {

            Swal.fire("PLease enter the Tester Suggestion minumum 10 characters or above");
            return false;
        }
    }
    ...
    if ($("[id*=hdEmpCheck]").val() == 0) {
        ...
        if (TestStatus == 1 || TestStatus == 6 || TestStatus == 5) {
            ...
            high = $("#txt_high_cnt").val().trim();
            medium = $("#txt_mid_cnt").val().trim();
            low = $("#txt_low_cnt").val().trim();

            if (high == "" || medium == "" || low == "") {
                alert("Please enter testcase count..!");
                return false;
            }
            // return false;
        }
            ...
            //alert($("#tabChange tr").length);

            if (TestStatus == 2 && $("#tabChange tr").length < 2 && $("[id *= hdnbug]").val() == 1) {

                alert("No Bug Found.! You Cannot ReAssinged to Developer.!");
                return false;
            }
            //else if (TestStatus == 2 && $("[id*=hdBugStatus]").val() == 0 && $("#tabChange tr").length < 2 && $("[id*=hdTestCheck]").val() != 0) {
            //    alert("No Bug Found.! You Cannot ReAssinged to Developer.!");
            //    return false;
            //}


            else if (TestStatus == 7 || TestStatus == 8) {
                if ($("#txtRemarks").val() == "") {
                    alert("Please add your remark before return to Developer.!");
                    return false;
                }
...
function updationcheck() {
    ...
    var CRFID = $("#ddlCrf").val();
    var sta = $("#ddlTestStatus").val();


    debugger;

    if (sta == "1" || sta == "6" || sta == "5") {

        $("#DivSuggestion").show();
        $("#DivTestCase").show();
        tsterdelaychk($("[id*=hdRqstID]").val(), CRFID);
...
function ConfirmTestResult() {
    ...
    var data = $("#ddlCrf").val() + '^' + rdbtn + '^' + $("#txtRemarks").val() + '^' + $("[id*=hdRqstID]").val() + '^' + $("[id*=hdUserId]").val() + '^' + $('#ddlTestStatus').val() + '^' + EmpCheck + '^' + sugges + '^' + high + '^' + medium + '^' + low;
    var itmdata = '';
