    ...
    $.ajax({
        ...
        success: function (Result) {
            ...
            if ($("#tblFilesTestCase tr").length == 0) {
                $('#tblFilesTestCase').append('<thead class="bg-success text-white"><tr><th scope="col">File No</th><th scope="col">File Name</th>><th class="text-left">DELETE</th></tr></thead>');
            }
    ...
    Swal.fire({
        ...
        if (result.value == true) {
            ...
            $.ajax({
                ...
                data: "{pageVal:'DeleteTestcase', pageval1:'" + noteid + "',pageval2:'" + usr + "',pageval3:'" + data + "'}",
                dataType: "json",
                success: function (Result) {
                    Result = Result.d;
                    // alert(Result);
...
function TestCaseUploadFile() {
    debugger;

    $("#TestCaseDocments").empty();
    ...
    $.ajax({
        ...
        url: "CRFTestCaseUpdation.aspx/getFileData",
        async: false,
        data: "{pageVal:'TestCaseUpdationper', pageval1 :'', pageval2 :'" + filenm + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            var valData, valData1;
            ...
            var n = 0;
            valData = Result.split('Θ');
            if ($("#TestCaseDocments tr").length == 0) {
                $('#TestCaseDocments').append('<thead class="bg-success text-white"><tr><th scope="col">File No</th><th scope="col">File Name</th></tr></thead>');
            }
            ...
            for (j = 0; j < valData.length - 1; j++) {
                ...
                $('#TestCaseDocments').append('<tbody><tr>' +
                    '<td>' + contentDtl[0] + '</td>' +
                    '<td><a href="' + myUrl + '" download="' + filename + '" class="file-list1">' + contentDtl[1] + '</a></td>' +
                    '</tr> </tbody>');
            }
    ...
    if ($("#TestCaseDocments tr").length > 1) {
        $("#TestCaseDoc").show();
    }
    else $("#TestCaseDoc").hide();
}