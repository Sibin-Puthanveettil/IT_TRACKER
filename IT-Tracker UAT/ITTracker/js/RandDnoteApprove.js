...
function crfload(sts) {
    ...
    //if (sts == 5) { //Head
        $.ajax({
            ...
            data: "{pageVal:'RandDApprovedTask', pageval1 :'" + usr + "'}",
            dataType: "json",
            success: function (Result) {
                Result = Result.d;
...
function WorkApproveDownload() {
    ...
    var usr = $("[id*=hdUserId]").val();
    var details =  $("#ddlCrf").val();//+ '^' + $("#txtModRemar").val();
   
        window.open("RandDApprovedView.aspx?mnuid=" + details + "", '_self');
