...
function ConfirmNote() {
    ...
    if ($('#usrBUG').prop('checked')) {
        alert("Please click return option");
        return false;
    }
...
function ReturnNote() {
    ...
    var inno = $('#ncdnt_No').val();

    if ($('#usrBUG').prop('checked')) {
        if (inno == "") {
            alert("Please enter a incident no..");
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                url: "userfeedback.aspx/getRequestNote",
                data: "{pageVal:'GetIncident', pageval1:'" + inno + "',pageval2:''}",
                dataType: "json",
                success: function (Result) {
                    Result = Result.d;
                    if (Result == '0') {
                        alert("Please enter a valid incident no..!");
                        return false;
                    }
                    else {
                        Swal.fire({
        ...
        alert("Please select production bug option");
        return false;
    }
...
function Returning() {
    ...
    var inno = $('#ncdnt_No').val();
    var data = crf + '§' + rqid + '§' + user + '§' + rmk + '§' + inno;
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
