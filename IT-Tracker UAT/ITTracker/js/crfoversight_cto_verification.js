...
function Accessibility() {
    ...
    $.ajax({
        ...
        url: "oversightreport_cto_verification.aspx/Accessibility",
        data: "{pageVal:'oversightheadaccess', pageval1:'" + usr + "'}",
        dataType: "json",
...
function CRFLoad() {
    ...
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "oversightreport_cto_verification.aspx/getFillData",
        data: "{pageVal:'ctoverificationload',pageval1:''}",
        dataType: "json",
        async: false,
        success: function (Result) {
            Result = Result.d;
...
function fillCRFData() {
    ...
    $("#tbl1").show();
    $("#cnfrm").show();
    var CRFIDD = $('#ddlCrf option:selected').text();
   
    requestid = CRFIDD.split('~')[1];
    
    Data = CRFID + 'æ' + requestid;
    getverifeddtls(Data);
}
function getverifeddtls(Data) {

    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "oversightreport_cto_verification.aspx/getFillData",
        data: "{pageVal:'getverifieddtls',pageval1:'" + Data + "'}",
        dataType: "json",
        async: false,
        success: function (Result) {
            Result = Result.d;
            $.each(Result, function (key, value) {
                $('#Verifiedby').val(value.id);
                $('#txtRemarks1').val(value.name);
...
function Confirmctoverification() {
    ...
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "oversightreport_cto_verification.aspx/confirm",
        data: "{pageVal:'crfoversightverification', pageval1 :'" + data + "', pageval2:''}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
            if (Result == '111') {
                ...
                Swal.fire({
                    ...
                    onClose: () => {
                        clearInterval(timerInterval)
                        window.open('oversightreport_cto_verification.aspx', '_self');
                    }
                }).then((result) => {
                    if (
                        result.dismiss === Swal.DismissReason.timer
